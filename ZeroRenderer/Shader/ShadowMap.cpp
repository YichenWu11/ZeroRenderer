...
ShadowMap::ShadowMap(ID3D12Device* device, UINT width, UINT height) : mWidth(width), mHeight(height)
{
	md3dDevice = device;
...
void ShadowMap::BuildResource()
{
	...
	texDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D; // 深度/模板缓冲区其实就是纹理 这里也类似
	texDesc.Alignment = 0;
	...
	ThrowIfFailed(md3dDevice->CreateCommittedResource(
		get_rvalue_ptr(CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT)), // default heap
